from ..common import BaseHandler, Server, asdict, generate_gravatar_url, generate_slug, get_one, hash_password, split_commas
...
class Article(NamedTuple):
  ...
  @classmethod
  def public_fields(cls):
    return ["article_id", "user_id", "article_title", "article_slug", "article_mtime", "short", "long"]
...
class Comment(NamedTuple):
  ...
  @classmethod
  def public_fields(cls):
    return ["comment_id", "user_id", "article_id", "comment_text", "comment_ctime"]
...
class UserController:
  ...
  @staticmethod
  def find(follower_id: int | None = None, followee_id: int | None = None, user_slug: str | None = None):
    join: list[str] = []
    ...
    if result:
      return User(*result[2:], total_follows=result[0], is_followed=bool(result[1]))
  ...
  def insert(form_data: dict[str, list[str]]) -> tuple[int | None, list[str]]:
    ...
    try:
      execute(
          ...
          VALUES (?, ?, ?, ?, ?, ?)
        """,
          (username, generate_slug(username), email,
           generate_gravatar_url(email), *hash_password(password))
      )
      commit()
    except:
  ...
  @staticmethod
  def sign_in(form_data: dict[str, list[str]]) -> tuple[int | None, list[str]]:
    errors: list[str] = []
    email = get_one(form_data, "email")
    password = get_one(form_data, "password")
    if not email:
  ...
  @staticmethod
  def update(user: User | None, form_data: dict[str, list[str]]) -> tuple[User | None, list[str]]:
    if not user:
...
class ArticleController:
  ...
  def get_by_article_slug(cls, user: User | None, article_slug: str | None):
    if not article_slug:
      return
    res = cls.search(user.user_id if user else None,
                     1, article_slug=article_slug)
  ...
  @staticmethod
  def search(user_id: int | None, page: int, feed: bool = False, tag_value: str | None = None, owner_id: int | None = None, liked: bool = False, article_slug: str | None = None):
    join: list[str] = []
    join_args: list[str | int] = []
    where: list[str] = []
    where_args: list[str | int] = []
    ...
    if tag_value:
      join.append("LEFT JOIN tags ON articles.article_id = tags.article_id")
    ...
    return [Article(
        *x[4: 4 + len(Article.public_fields())],
        ...
        is_liked=bool(x[1]),
        author=User(
            *x[4 + len(Article.public_fields()): 4 +
               len(Article.public_fields()) + len(User.public_fields())],
            total_follows=x[2],
  ...
  @classmethod
  def update(cls, user: User | None, article_slug: str | None, form_data: dict[str, list[str]]) -> tuple[Article | None, list[str]]:
    errors: list[str] = []
    ...
    short = f"{
        short[0:max_len - len(suffix)]}{suffix}" if len(short) > max_len else short
    ...
    commit()
    return article._replace(article_title=article_title, article_slug=slug, short=short, long=long, article_mtime=int(time())), []

  @staticmethod
  def insert(user: User | None, form_data: dict[str, list[str]]) -> tuple[Article | None, list[str]]:
    errors: list[str] = []
    article_title = get_one(form_data, "article_title")
    long = get_one(form_data, "long")
    tags = split_commas(get_one(form_data, "tags"))
    ...
    if article_id:
      ...
      commit()
      return Article(article_id, user.user_id, article_title, f"{slug}-{article_id}", int(time()), short, long, 0, False, user), []
...
class CommentController:
  ...
  @classmethod
  def insert(cls, user: User | None, article_id: int, form_data: dict[str, list[str]]) -> tuple[Comment | None, list[str]]:
    errors: list[str] = []
    ...
    if comment_id:
      return Comment(comment_id, user.user_id, article_id, comment_text, int(time()), user), []
...
class LikeController:
  ...
  @staticmethod
  def insert(user: User | None, article_id: int) -> tuple[dict[str, Any] | None, str]:
    if not user:
    ...
    return {"article_id": article_id, "total_likes": result[0] if result else 0, "is_liked": True}, ""

  @staticmethod
  def remove(user: User | None, article_id: int) -> tuple[dict[str, Any] | None, str]:
    if not user:
      return None, "Need to be logged in to remove an article from favorites"
    execute(
        """
        DELETE FROM likes
        WHERE likes.user_id = ?
    ...
    return {"article_id": article_id, "total_likes": result[0] if result else 0, "is_liked": False}, ""


class FollowController:

  @staticmethod
  def insert(user: User | None, user_id: int) -> tuple[dict[str, Any] | None, str]:
    if not user:
    ...
    return {"author": Followee(user_id, result[1] if result else "", result[0] if result else 0, True)}, ""

  @staticmethod
  def remove(user: User | None, user_id: int) -> tuple[dict[str, Any] | None, str]:
    if not user:
    ...
    return {"author": Followee(user_id, result[1] if result else "", result[0] if result else 0, False)}, ""
...
class Handler(BaseHandler):
  ...
  @property
  def ctx_tag(self):
    return self.parsed_params.get("tag")
  ...
  @property
  def ctx_author(self):
    return UserController.find(follower_id=self.user.user_id if self.user else None, user_slug=self.ctx_slug)
  ...
  def ctx_global_articles(self):
    return ArticleController.search(self.user.user_id if self.user else None, self.ctx_page)

  @property
  def ctx_feed_articles(self):
    return ArticleController.search(self.user.user_id if self.user else None, self.ctx_page, feed=True)

  @property
  def ctx_tag_articles(self):
    return ArticleController.search(self.user.user_id if self.user else None, self.ctx_page, tag_value=self.ctx_tag)

  @property
  def ctx_author_articles(self):
    return ArticleController.search(self.user.user_id if self.user else None, self.ctx_page, owner_id=self.ctx_author.user_id if self.ctx_author else None)

  @property
  def ctx_liked_articles(self):
    return ArticleController.search(self.user.user_id if self.user else None, self.ctx_page, owner_id=self.ctx_author.user_id if self.ctx_author else None, liked=True)
  ...
  def get_create_auth(self):
    self.send_tpl("index", content="editor", article_slug="",
                  article_title="", short="", long="", tags=[])

  def get_create_xhr(self):
    self.get_login_xhr()

  def get_create_xhr_auth(self):
    self.send_tpl("content", content="editor", article_slug="",
                  article_title="", short="", long="", tags=[])
  ...
  def get_editor_auth(self):
    article = ArticleController.get_by_article_slug(self.user, self.ctx_slug)
    if article:
  ...
  def post_split_chips(self):
    self.send_tpl("chips", tags=split_commas(self.single_parsed_data("tags")))
  ...
  def delete_split_chips(self):
    self.send_tpl("articleTags", tags=split_commas(
        self.single_parsed_query("chip")))
